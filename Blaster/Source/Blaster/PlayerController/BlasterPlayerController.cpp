...
#include "Blaster/HUD/ReturnToMainMenuWidget.h"
...
void ABlasterPlayerController::CheckPing(float DeltaTime)
{
	...
	if (HighPingRunningTime > CheckPingFrequency)
	{
		...
		if (PlayerState)
		{
			...
			if (PlayerState->GetCompressedPing() * 4 > HighPingThreshold) // ping is compressed, it is actually ping / 4
			{
				HighPingWarning();
...
void ABlasterPlayerController::PollInit()
{
	if (CharacterOverlay == nullptr)
	{
		if (BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if (CharacterOverlay)
			{
				if (bIntializeHealth) SetHUDHealth(HUDHealth, HUDMaxHealth);
				if (bIntializeShield) SetHUDShield(HUDShield, HUDMaxShield);
				if (bIntializeScore) SetHUDScore(HUDScore);
				if (bIntializeDefeats) SetHUDDefeats(HUDDefeats);
				...
				{
					if (bIntializeGrenades) SetHUDGrenades(BlasterCharacter->GetCombat()->GetGrenades());
				}
			}
		}
	}
...
FString ABlasterPlayerController::GetInfoText(const TArray<class ABlasterPlayerState*>& Players)
{
	...
	{
		InfoTextString = FString::Printf(TEXT("%s is the winner"), *Players[0]->GetPlayerName());
	}
	else if (Players.Num() > 1)
	{
