...
void UCombatComponent::EquipSecondaryWeapon(AWeapon* WeaponToEquip)
{
	...
	AttachActorToBagpack(WeaponToEquip);
	PlayEquipWeaponSound(WeaponToEquip);
...
void UCombatComponent::AttachActorToBagpack(AActor* ActorToAttach)
{
	if (Character == nullptr || Character->GetMesh() == nullptr || ActorToAttach == nullptr) return;
...
void UCombatComponent::OnRep_SecondaryWeapon()
{
	...
	{
		SecondaryWeapon->SetWeaponState(EWeaponState::EWS_EquippedSecondary);
		AttachActorToBagpack(SecondaryWeapon);
		PlayEquipWeaponSound(EquippedWeapon);
	}
}
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			if (EquippedWeapon && bFireButtonPressed && CanFire())
			{
				CrosshairShootingFactor = FMath::FInterpTo(CrosshairShootingFactor, .75f, DeltaTime, 30.f);
			}
			else
			{
