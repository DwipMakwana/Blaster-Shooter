...
FServerSideRewindResult ULagCompensationComponent::ConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation)
{
	...
	EnableCharacterMeshCollisions(HitCharacter, ECollisionEnabled::NoCollision);
	...
	if (World)
	{
		...
		if (ConfirmHitResult.bBlockingHit) //  We hit the head return early
		{
			...
			EnableCharacterMeshCollisions(HitCharacter, ECollisionEnabled::QueryAndPhysics);
			return FServerSideRewindResult{true, true};
		}
		else//	Didn't hit the head so check rest of the boxes
		{
...
FServerSideRewindResult ULagCompensationComponent::ProjectileConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	else // we didnt hit the head, check rest of the boxes
	{
		for (auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
		{
			if (HitBoxPair.Value != nullptr)
...
FShotgunServerSideRewindResult ULagCompensationComponent::ShotgunConfirmHit(const TArray<FFramePackage>& FramePackages, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	for (auto& Frame : FramePackages)
	{
		...
		EnableCharacterMeshCollisions(Frame.Character, ECollisionEnabled::NoCollision);
	...
	for (auto& Frame : CurrentFrames)
	{
		...
		EnableCharacterMeshCollisions(Frame.Character, ECollisionEnabled::QueryAndPhysics);
	}
...
void ULagCompensationComponent::EnableCharacterMeshCollisions(ABlasterCharacter* HitCharacter, ECollisionEnabled::Type CollisionEnabled)
{
...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	//	Frame package verify to check a hit
	FFramePackage FrameToCheck;
	bool bShouldInterpolate = true;
	//	Frame history of the HitCharacter
	const TDoubleLinkedList<FFramePackage>& History = HitCharacter->GetLagCompensation()->FrameHistory;
	const float OldestHistroyTime = History.GetTail()->GetValue().Time;
	const float NewestHistroyTime = History.GetHead()->GetValue().Time;
	if (OldestHistroyTime > HitTime)
	{
	...
	{
		// Intrerpolate between younger and older
...
void ULagCompensationComponent::SaveFramePackage()
{
	...
	else
	{
		...
		//ShowFramePackage(ThisFrame, FColor::Yellow);
	}
}
